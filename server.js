...
const DEFAULT_MODEL     = 'claude-haiku-4-5-20251001'; // el más económico

// Precios USD por token (actualizar según pricing de Anthropic)
const PRICING = {
  input:  0.80  / 1_000_000,   // $0.80 por millón de tokens de entrada
  output: 4.00  / 1_000_000,   // $4.00 por millón de tokens de salida
};
...
app.post('/api/admin/users', requireAuth, requireAdmin, (req, res) => {
  ...
  try {
    ...
    const result = db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?)
    `).run(name, email, hash, daily_limit_usd ?? 1.00, monthly_limit_usd ?? 10.00);
